...
class TestFilter:
    ...
    def setup(self):
        ...
        with patch('patriot_center_backend.dynamic_filtering.dynamic_filter.CACHE_MANAGER.get_players_cache') as mock_get_players_cache, \
             patch('patriot_center_backend.dynamic_filtering.dynamic_filter.validate_dynamic_filter_args') as mock_validate, \
             patch('patriot_center_backend.dynamic_filtering.dynamic_filter.find_valid_years') as mock_find_years, \
             patch('patriot_center_backend.dynamic_filtering.dynamic_filter.find_valid_weeks') as mock_find_weeks, \
             patch('patriot_center_backend.dynamic_filtering.dynamic_filter.find_valid_managers') as mock_find_managers, \
             patch('patriot_center_backend.dynamic_filtering.dynamic_filter.find_valid_positions') as mock_find_positions, \
             patch('patriot_center_backend.dynamic_filtering.dynamic_filter.format_output') as mock_format:
    ...
    def test_calls_validate_with_all_args(self):
        ...
        self.mock_validate.assert_called_once_with("2024", "1", "Tommy", "QB", None)
    ...
    def test_calls_find_valid_weeks_with_filters(self):
        ...
        self.mock_find_weeks.assert_called_once_with("2024", "Tommy", "QB", None)
    ...
    def test_locks_position_when_player_provided(self):
        ...
        self.mock_find_years.assert_called_once_with(None, "QB", "Jayden Daniels")
    ...
    def test_player_position_used_in_all_find_calls(self):
        ...
        # Jayden Daniels's position is QB, so QB should be passed to find functions
        self.mock_find_years.assert_called_once_with("Tommy", "QB", "Jayden Daniels")
        self.mock_find_weeks.assert_called_once_with("2024", "Tommy", "QB", "Jayden Daniels")
        self.mock_find_managers.assert_called_once_with("2024", "1", "QB", "Jayden Daniels")

    def test_format_output_receives_player_position_set(self):
        """When player provided, format_output receives set with player's position."""
        filter(
            year=None,
            week=None,
            manager=None,
