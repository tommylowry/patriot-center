...
def check_for_reverse_transactions(transaction_ids: List[str]) -> None:
    ...
    while len(transaction_ids) > 1:
        ...
        for transaction_id2 in transaction_ids:
            ...
            if transaction1["commish_action"]:
                if "add" in transaction1:
                    if transaction1["add"] == transaction2.get("drop", ""):
                        trans_1_removed = revert_add_drop_transaction(transaction_id1, "add", transaction_ids)
                        trans_2_removed = revert_add_drop_transaction(transaction_id2, "drop", transaction_ids)
                ...
                if "drop" in transaction1:
                    if transaction1["drop"] == transaction2.get("add", ""):
                        trans_1_removed = revert_add_drop_transaction(transaction_id1, "drop", transaction_ids)
                        trans_2_removed = revert_add_drop_transaction(transaction_id2, "add", transaction_ids)
            ...
            revert_trade_transaction(transaction_id1, transaction_id2, transaction_ids)
            break
