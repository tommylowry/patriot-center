...
def validate_caching_preconditions(
    ...
    if len(weekly_roster_ids) % 2 == 1:
        ...
        raise ValidationError(
            "Odd number of roster IDs cached, cannot process matchups."
        )
...
def validate_matchup_data(matchup_data: dict[str, Any] | None) -> str:
    ...
    Returns:
        ...
        - `"Warning, *"` with details if validation issues found
    """
    # No matchup data means manager didn't
    # play that week (e.g., not in playoffs)
    if not matchup_data:
        return "Empty"
    ...
    if points_against <= 0.0:
        return (
            f"Warning, invalid points_against {points_against} in matchup_data"
        )
    ...
    if result == "win" and points_for < points_against:
        return (
            f"Warning, result is win but points_against {points_against} "
            f"is more than points_for {points_for} in matchup_data"
        )
    if result == "loss" and points_for > points_against:
        return (
            f"Warning, result is loss but points_for {points_for} "
            f"is more than points_against {points_against} in matchup_data"
        )
    if result == "tie" and points_for != points_against:
        return (
            f"Warning, result is tie but points_for {points_for} is not "
            f"the same as points_against {points_against} in matchup_data"
        )
...
def validate_transaction(
    ...
    Args:
        ...
        roster_ids: Roster ID to manager mapping
    ...
    if transaction.get("status", "") != "complete":
        logger.warning(f"Unexpected transaction status: {transaction}")
        return False

    # Validate transaction type
    if transaction_type not in {"trade", "add_or_drop"}:
        logger.warning(f"Unexpected transaction type: {transaction}")
        return False
    ...
    if transaction_type == "trade":
        ...
        if "transaction_id" not in transaction:
            logger.warning(
                f"Invalid trade transaction "
                f"(missing transaction_id): {transaction}"
            )
            return False

        # Trades must involve at least 2 rosters
        if (
            "roster_ids" not in transaction
            or len(transaction.get("roster_ids", {})) < 2
        ):
            logger.warning(
                f"Invalid trade transaction (missing roster_ids): {transaction}"
            )
            return False
        ...
        if "adds" not in transaction or "drops" not in transaction:
            logger.warning(
                f"Invalid trade transaction (missing adds/drops): {transaction}"
            )
            return False
...
def validate_manager_query(manager: str, year: str | None) -> None:
    ...
    if not manager_cache[manager].get("years", {}):
        raise ValueError(
            f"No years found for manager {manager} in cache."
        )
