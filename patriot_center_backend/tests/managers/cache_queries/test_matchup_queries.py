...
class TestGetMatchupDetailsFromCache:
    ...
    def setup(self, mock_manager_cache):
        ...
        with patch('patriot_center_backend.managers.cache_queries.matchup_queries.CACHE_MANAGER.get_manager_cache') as mock_get_manager:
...
class TestGetOverallDataDetailsFromCache:
    ...
    def setup(self, mock_manager_cache):
        """Setup common mocks for all tests."""
        with patch('patriot_center_backend.managers.cache_queries.matchup_queries.CACHE_MANAGER.get_manager_cache') as mock_get_manager, \
             patch('patriot_center_backend.managers.cache_queries.matchup_queries.get_matchup_card') as mock_get_matchup_card:
    ...
    def test_week_selection_for_year_2020_and_earlier(self):
        ...
        self.mock_manager_cache["Manager 1"]["summary"]["overall_data"]["placement"]["2020"] = 2
        self.mock_manager_cache["Manager 1"]["years"]["2020"] = {
    ...
    def test_missing_opponent_skips_matchup_card(self, capsys):
    ...
    def test_get_overall_data_immutable(self, capsys):
        """Test that function doesn't modify cache."""
        original = deepcopy(self.mock_manager_cache)
        ...
        # Verify warning was printed for missing opponent
        captured = capsys.readouterr()
        assert "unable to retreive opponent" in captured.out
        assert "year 2023 week 17" in captured.out
        assert "year 2022 week 17" in captured.out
