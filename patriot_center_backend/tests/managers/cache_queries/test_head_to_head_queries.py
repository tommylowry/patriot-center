...
class TestGetHeadToHeadDetailsFromCache:
    ...
    def setup(self, mock_manager_cache):
        ...
        with patch('patriot_center_backend.managers.cache_queries.head_to_head_queries.CACHE_MANAGER.get_manager_cache') as mock_get_manager, \
             patch('patriot_center_backend.managers.cache_queries.head_to_head_queries.get_image_url') as mock_get_image_url:
...
class TestGetHeadToHeadOverallFromCache:
    ...
    def setup(self, mock_manager_cache):
        """Setup common mocks for all tests."""
        with patch('patriot_center_backend.managers.cache_queries.head_to_head_queries.CACHE_MANAGER.get_manager_cache') as mock_get_manager, \
             patch('patriot_center_backend.managers.cache_queries.head_to_head_queries.get_matchup_card') as mock_get_matchup_card, \
             patch('patriot_center_backend.managers.cache_queries.head_to_head_queries.validate_matchup_data') as mock_validate, \
             patch('patriot_center_backend.managers.cache_queries.head_to_head_queries.get_head_to_head_details_from_cache') as mock_h2h_details:
    ...
    def test_h2h_overall_stats(self, capsys):
        ...
        assert "No victories found for Manager 2 against Manager 1" in captured.out
        assert "Cannot compute average margin of victory" in captured.out
    ...
    def test_h2h_no_matchups(self, capsys):
        """Test H2H when managers never played."""
        result = get_head_to_head_overall_from_cache("Manager 1", "Manager 3", {})

        captured = capsys.readouterr()
        assert "No victories found for Manager 1 against Manager 3" in captured.out
        assert "No victories found for Manager 3 against Manager 1" in captured.out
        assert "Cannot compute average margin of victory" in captured.out
    ...
    def test_h2h_with_specific_year(self, capsys):
        ...
        self.mock_manager_cache["Manager 1"]["years"]["2023"]["summary"]["matchup_data"]["overall"]["points_for"]["opponents"]["Manager 2"] = 120.5
        self.mock_manager_cache["Manager 2"] = deepcopy(self.mock_manager_cache["Manager 1"])
        self.mock_manager_cache["Manager 2"]["years"]["2023"]["summary"]["matchup_data"]["overall"]["points_for"]["opponents"] = {"Manager 1": 100.0}
    ...
    def test_h2h_manager2_wins(self, capsys):
        ...
        self.mock_matchup_card_value = {"year": "2023", "week": "1", "margin": 20.0}

        result = get_head_to_head_overall_from_cache("Manager 1", "Manager 2", {})

        captured = capsys.readouterr()
        assert "No victories found for Manager 1 against Manager 2" in captured.out
        assert "Cannot compute average margin of victory" in captured.out
